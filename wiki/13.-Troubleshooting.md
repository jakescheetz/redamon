...
Common issues and their solutions when running Parallax.
...
Parallax with the full GVM stack requires significant resources. If containers are being killed:
...
```bash
docker compose down
docker volume rm parallax_postgres_data
docker compose up -d
```
...
```bash
docker compose down
docker volume rm parallax_neo4j_data
docker compose up -d
```
...
To completely reset Parallax and start fresh:
...
- **GitHub Issues:** [github.com/samugit83/parallax/issues](https://github.com/samugit83/parallax/issues) — report bugs or request features
