...
Projects are the core organizational unit in Parallax. Each project represents a single target domain and contains all the configuration for how to scan, analyze, and test it. This page walks you through creating a project with the 11-tab configuration form.
...
| **Target Domain** | The root domain to scan (e.g., `example.com`). Parallax checks for conflicts with existing projects |
...
| **TXT Record Prefix** | `_parallax-verify` | The DNS record name prefix (e.g., `_parallax-verify.example.com`) |
...
2. Parallax checks for domain conflicts (no two projects can target the same domain)
...
Parallax prevents multiple projects from targeting the same domain. When you enter a target domain:
