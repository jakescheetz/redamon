...
def run_resource_enum(
    recon_data: dict, output_file: Optional[Path] = None, settings: dict = None
) -> dict:
    """
    ...
    """
    print("\n" + "=" * 70)
    print("         parallax - Resource Enumeration")
    ...
    KATANA_DOCKER_IMAGE = settings.get(
    ...
    GAU_BLACKLIST_EXTENSIONS = settings.get(
    ...
    GAU_VERIFY_DOCKER_IMAGE = settings.get(
    ...
    GAU_VERIFY_ACCEPT_STATUS = settings.get(
    ...
    KITERUNNER_IGNORE_STATUS = settings.get(
        "KITERUNNER_IGNORE_STATUS", ["404", "429", "503"]
    )
    ...
    KITERUNNER_METHOD_DETECTION_MODE = settings.get(
        "KITERUNNER_METHOD_DETECTION_MODE", "options"
    )
    KITERUNNER_BRUTEFORCE_METHODS = settings.get(
        "KITERUNNER_BRUTEFORCE_METHODS", ["GET", "POST", "PUT", "DELETE", "PATCH"]
    )
    KITERUNNER_METHOD_DETECT_TIMEOUT = settings.get(
        "KITERUNNER_METHOD_DETECT_TIMEOUT", 3
    )
    KITERUNNER_METHOD_DETECT_RATE_LIMIT = settings.get(
        "KITERUNNER_METHOD_DETECT_RATE_LIMIT", 50
    )
    KITERUNNER_METHOD_DETECT_THREADS = settings.get(
        "KITERUNNER_METHOD_DETECT_THREADS", 20
    )
        ...
        for name, future in futures.items():
            try:
                if name == "katana":
                    ...
                    gau_urls, gau_urls_by_domain = future.result(
                        timeout=GAU_TIMEOUT * len(GAU_PROVIDERS) + 180
                    )
    ...
    if KITERUNNER_ENABLED and target_urls and kr_binary_path and KITERUNNER_WORDLISTS:
        ...
        for wordlist_name in KITERUNNER_WORDLISTS:
            ...
            try:
                ...
                print(
                    f"    [+] {wordlist_name}: {len(wordlist_results)} endpoints found, {len(kr_results)} total unique"
                )
            except Exception as e:
    ...
    if GAU_ENABLED and gau_urls:
        ...
        if GAU_VERIFY_URLS:
            print(
                f"    [+] GAU skipped (unverified): {gau_stats['gau_skipped_unverified']}"
            )
        if GAU_DETECT_METHODS:
            print(f"    [+] GAU with POST method: {gau_stats.get('gau_with_post', 0)}")
            print(
                f"    [+] GAU with multiple methods: {gau_stats.get('gau_with_multiple_methods', 0)}"
            )
        if GAU_FILTER_DEAD_ENDPOINTS:
            print(
                f"    [+] GAU dead endpoints filtered: {gau_stats.get('gau_skipped_dead', 0)}"
            )
    ...
    if KITERUNNER_ENABLED and kr_results:
        ...
        organized_data["by_base_url"], kr_stats = merge_kiterunner_into_by_base_url(
            kr_results, organized_data["by_base_url"], kr_url_methods
        )
        ...
        if (
            KITERUNNER_DETECT_METHODS
            and kr_stats.get("kr_with_multiple_methods", 0) > 0
        ):
            print(
                f"    [+] Endpoints with multiple methods: {kr_stats['kr_with_multiple_methods']}"
            )
    ...
    resource_enum_result = {
        "scan_metadata": {
            ...
            "gau_method_detection_enabled": (
                GAU_DETECT_METHODS if GAU_ENABLED else False
            ),
            "gau_filter_dead_endpoints": (
                GAU_FILTER_DEAD_ENDPOINTS if GAU_ENABLED else False
            ),
            "gau_stats": gau_stats,
            # Kiterunner metadata
            ...
            "kiterunner_wordlists_count": (
                len(KITERUNNER_WORDLISTS) if KITERUNNER_ENABLED else 0
            ),
            "kiterunner_endpoints_found": len(kr_results) if KITERUNNER_ENABLED else 0,
            "kiterunner_method_detection_enabled": (
                KITERUNNER_DETECT_METHODS if KITERUNNER_ENABLED else False
            ),
            "kiterunner_method_detection_mode": (
                KITERUNNER_METHOD_DETECTION_MODE if KITERUNNER_ENABLED else None
            ),
        ...
        "summary": {
            ...
            "kiterunner_with_multiple_methods": kr_stats.get(
                "kr_with_multiple_methods", 0
            ),
            "methods": {},
            "categories": {},
    ...
    print(
        f"    - Katana (active crawl): {len(katana_urls) if KATANA_ENABLED else 'disabled'}"
    )
    print(
        f"    - GAU (passive archive): {len(gau_urls) if GAU_ENABLED else 'disabled'}"
    )
    if GAU_ENABLED and gau_urls:
        print(f"      - GAU new endpoints: {gau_stats['gau_new']}")
        print(f"      - GAU overlap: {gau_stats['gau_overlap']}")
    print(
        f"    - Kiterunner (API bruteforce): {len(kr_results) if KITERUNNER_ENABLED else 'disabled'}"
    )
    if KITERUNNER_ENABLED and kr_results:
        print(f"      - Kiterunner new endpoints: {kr_stats['kr_new']}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if recon_file.exists():
            ...
            result = run_resource_enum(
                recon_data, output_file=recon_file, settings=settings
            )
