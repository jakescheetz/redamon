"""
parallax - Docker Helper Functions
=================================
...
def ensure_templates_volume(docker_image: str, auto_update: bool = False) -> bool:
    ...
    try:
        ...
        if not volume_exists:
            ...
            check_result = subprocess.run(
                [
                    ...
                    "sh",
                    "-c",
                    "find /root/nuclei-templates -name '*.yaml' 2>/dev/null | head -5 | wc -l",
                ],
                capture_output=True,
                text=True,
                timeout=30,
            ...
            template_count = (
                int(check_result.stdout.strip())
                if check_result.stdout.strip().isdigit()
                else 0
            )
        ...
        if needs_download:
            print(
                f"    [*] Downloading nuclei templates (first run, this may take a minute)..."
            )
        elif auto_update:
        ...
        if needs_download or auto_update:
            ...
            if update_result.returncode != 0:
                ...
                if update_result.stderr:
                    ...
                    errors = [
                        l
                        for l in update_result.stderr.split("\n")
                ...
                if update_result.stdout:
                    for line in update_result.stdout.split("\n"):
                        if (
                            "Successfully updated" in line
                            or "already up to date" in line.lower()
                        ):
