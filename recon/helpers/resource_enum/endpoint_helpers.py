"""
parallax - Endpoint Organization Helpers
=======================================
...
def organize_endpoints(discovered_urls: List[str], use_proxy: bool = False) -> Dict:
    """
    ...
    all_forms = fetch_forms_from_urls(
        discovered_urls, use_proxy=use_proxy, max_urls=100
    )
    ...
    for form in all_forms:
        ...
        for input_field in form["inputs"]:
            ...
            existing_param = next(
                (
                    p
                    for p in endpoint["parameters"]["body"]
                    if p["name"] == input_field["name"]
                ),
                None,
            )
    ...
    for base_url, paths in by_base_url.items():
        ...
        for path, endpoint in paths.items():
            ...
            category = classify_endpoint(
                path, endpoint["methods"], endpoint["parameters"]
            )
            ...
            endpoints_by_base[base_url]["summary"]["categories"][category] = (
                endpoints_by_base[base_url]["summary"]["categories"].get(category, 0)
                + 1
            )

    return {"by_base_url": endpoints_by_base, "forms": all_forms}
