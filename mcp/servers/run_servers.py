#!/usr/bin/env python3
"""
MCP Server Runner - Launches all MCP servers for parallax Agentic AI
...
SERVERS = {
    ...
    "nmap": {"module": "nmap_server", "port": 8004, "description": "Network Mapper"},
}
...
def run_server(name: str, config: dict, transport: str = "sse"):
    ...
    logger.info(
        f"Starting {name} server ({config['description']}) on port {config['port']}"
    )
    ...
    try:
        ...
        if transport == "sse":
            ...
            if name == "metasploit" and hasattr(module, "start_progress_server"):
                ...
                logger.info(
                    f"Started metasploit progress server on port {progress_port}"
                )

            # Start progress server for network_recon (for Hydra live progress updates)
            if name == "network_recon" and hasattr(
                module, "start_hydra_progress_server"
            ):
                hydra_progress_port = int(os.getenv("HYDRA_PROGRESS_PORT", "8014"))
                module.start_hydra_progress_server(hydra_progress_port)
                logger.info(
                    f"Started Hydra progress server on port {hydra_progress_port}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="parallax MCP Server Runner")
    parser.add_argument(
        "--stdio",
        action="store_true",
